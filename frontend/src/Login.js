...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (!username || !password) {
      setModal({ icon: '⚠️', title: 'MISSING FIELDS', message: 'Please enter both username and password.' });
      return;
    }
    setLoading(true);
    try {
      ...
      if (res.ok) {
        ...
        setModal({ icon: '❌', title: 'LOGIN FAILED', message: data.error || 'Invalid credentials.' });
      }
    } catch {
