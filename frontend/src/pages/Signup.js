...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (!username || !email || !password || !confirm) {
      setModal({ icon: 'âš ï¸', title: 'MISSING FIELDS', message: 'Please fill in all fields.' });
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setModal({ icon: 'ðŸ“§', title: 'INVALID EMAIL', message: 'Please enter a valid email address.' });
      return;
    }
    if (password.length < 6) {
      setModal({ icon: 'ðŸ”’', title: 'WEAK PASSWORD', message: 'Password must be at least 6 characters.' });
      return;
    }
    if (password !== confirm) {
      setModal({ icon: 'âŒ', title: 'MISMATCH', message: 'Passwords do not match.' });
      return;
    }
    ...
    try {
      ...
      if (res.ok || res.status === 201) {
        setModal({ title: 'ACCOUNT CREATED', message: 'You can now sign in with your credentials.' });
        setTimeout(() => navigate('/login'), 1800);
        setForm({ username: '', email: '', password: '', confirm: '' });
      } else {
        const msg = data.username?.[0] || data.email?.[0] || data.error || 'Registration failed.';
        setModal({ icon: 'âŒ', title: 'SIGNUP FAILED', message: msg });
      }
    } catch {
