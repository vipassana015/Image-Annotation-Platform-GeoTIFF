...
class RestoreProjectView(APIView):
    ...
    def patch(self, request, project_id):
        project = get_object_or_404(Project, id=project_id, is_deleted=True)
        if not is_owner_or_admin(request.user, project):
            return Response({"detail": "Permission denied."}, status=403)
...
class PermanentDeleteProjectView(APIView):
    permission_classes = [permissions.IsAuthenticated]
    ...
    def delete(self, request, project_id):
        ...
        if not is_owner_or_admin(request.user, project):
            return Response({"detail": "Permission denied."}, status=403)
        project.delete()
        return Response({"detail": "Project permanently deleted."})


class TrashedProjectsView(APIView):
    permission_classes = [permissions.IsAuthenticated]
